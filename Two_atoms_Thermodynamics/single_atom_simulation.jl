...
B_cav = FockBasis(Nc)
B_atom1 = NLevelBasis(3)
B_atom2 = NLevelBasis(3)
B = B_atom1 ⊗ B_atom2 ⊗ B_cav

# Operators cavity
a = one(B_atom1) ⊗ one(B_atom2) ⊗ destroy(B_cav)
at = one(B_atom1) ⊗ one(B_atom2) ⊗ create(B_cav)
numc = at * a 

# Operators, atom 1

p1 = transition(B_atom1,1,1) ⊗ one(B_atom2) ⊗ one(B_cav);
p2 = transition(B_atom1,2,2) ⊗ one(B_atom2) ⊗ one(B_cav);
p3 = transition(B_atom1,3,3) ⊗ one(B_atom2) ⊗ one(B_cav);

σ = transition(B_atom1, 1, 2) ⊗ one(B_atom2) ⊗ one(B_cav)
σt = dagger(σ)

# Dissipator Operators - Atom 1
# Hot bath: 1 <-> 3
σ13 = transition(B_atom1, 1, 3) ⊗ one(B_atom2) ⊗ one(B_cav) # |1><3|
σ31 = dagger(σ13)                                           # |3><1|

# Cold bath: 2 <-> 3
σ23 = transition(B_atom1, 2, 3) ⊗ one(B_atom2) ⊗ one(B_cav) # |2><3|
σ32 = dagger(σ23)                                           # |3><2|

# Operators, atom 2

p1_2 = one(B_atom1) ⊗ transition(B_atom2,1,1) ⊗ one(B_cav);
p2_2 = one(B_atom1) ⊗ transition(B_atom2,2,2) ⊗ one(B_cav);
p3_2 = one(B_atom1) ⊗ transition(B_atom2,3,3) ⊗ one(B_cav);

σ_2 = one(B_atom1) ⊗ transition(B_atom2, 1, 2) ⊗ one(B_cav)
σt_2 = dagger(σ_2)

# Dissipator Operators - Atom 2
# Hot bath: 1 <-> 3
σ13_2 = one(B_atom1) ⊗ transition(B_atom2, 1, 3) ⊗ one(B_cav) # |1><3|
σ31_2 = dagger(σ13_2)                                         # |3><1|

# Cold bath: 2 <-> 3
σ23_2 = one(B_atom1) ⊗ transition(B_atom2, 2, 3) ⊗ one(B_cav) # |2><3|
σ32_2 = dagger(σ23_2)                                         # |3><2|

# Hamiltonian (both atoms couple to cavity symmetrically)
function H(x)
    return f*(at*σ+a*σt) + f*(at*σ_2+a*σt_2) - sqrt(2)*g*numc*x;
end

# Jump Operators, different for zero or finite cold temperatures
# Include jumps for both atoms
if n_c == 0
    J = [sqrt(κ*(n_c+1))*a,
         sqrt(Γ_h*(n_h+1))*σ13, sqrt(Γ_h*n_h)*σ31, sqrt(Γ_c*(n_c+1))*σ23,
         sqrt(Γ_h*(n_h+1))*σ13_2, sqrt(Γ_h*n_h)*σ31_2, sqrt(Γ_c*(n_c+1))*σ23_2]
    Jnj = [sqrt(κ*(n_c+1))*a,
           sqrt(Γ_h*(n_h+1))*σ13, sqrt(Γ_h*n_h)*σ31,
           sqrt(Γ_h*(n_h+1))*σ13_2, sqrt(Γ_h*n_h)*σ31_2] # jumps excluding the monitored ones
else
    J = [sqrt(κ*(n_c+1))*a, sqrt(κ*n_c)*at,
         sqrt(Γ_h*(n_h+1))*σ13, sqrt(Γ_h*n_h)*σ31, sqrt(Γ_c*(n_c+1))*σ23, sqrt(Γ_c*n_c)*σ32,
         sqrt(Γ_h*(n_h+1))*σ13_2, sqrt(Γ_h*n_h)*σ31_2, sqrt(Γ_c*(n_c+1))*σ23_2, sqrt(Γ_c*n_c)*σ32_2]
    Jnj = [sqrt(κ*(n_c+1))*a, sqrt(κ*n_c)*at,
           sqrt(Γ_h*(n_h+1))*σ13, sqrt(Γ_h*n_h)*σ31, sqrt(Γ_c*n_c)*σ32,
           sqrt(Γ_h*(n_h+1))*σ13_2, sqrt(Γ_h*n_h)*σ31_2, sqrt(Γ_c*n_c)*σ32_2] # jumps excluding the monitored ones
end

# Monitored jump operators (both atoms' cold transitions)
Jc = [sqrt(Γ_c*(n_c+1))*σ23, sqrt(Γ_c*(n_c+1))*σ23_2]
...
if ur == 0
    ψ0 = nlevelstate(B_atom1,1) ⊗ nlevelstate(B_atom2,1) ⊗ fockstate(B_cav,0);
    ρ0 = tensor(ψ0,dagger(ψ0));
    xm0 = -25; # initial mechanical position. 
    pm0 = 0;
...
global p1_avg = []; # occupation state 1 (atom 1)
global p2_avg = []; # occupation state 2 (atom 1)
global p3_avg = []; # occupation state 3 (atom 1)

global p1_2_avg = []; # occupation state 1 (atom 2)
global p2_2_avg = []; # occupation state 2 (atom 2)
global p3_2_avg = []; # occupation state 3 (atom 2)
...
for (step_idx, t) in enumerate(tvec)
    ...
    if t > j*Δt
        ...
        push!(p1_2_avg,real(expect(p1_2, ρ)))
        push!(p2_2_avg,real(expect(p2_2, ρ)))
        push!(p3_2_avg,real(expect(p3_2, ρ)))
...
# Heat currents (compute time-averaged values for steady state)
ω13 = params["w_hot"]  # Hot bath transition frequency
ω23 = α * ω13  # Cold bath transition frequency (α=2 defined at top)

# Time-average over the last portion (steady state)
steady_start_idx = max(1, Int(floor(0.8*length(p3_avg))))  # Last 20% of trajectory

# Compute heat currents (scalar, time-averaged) - sum over both atoms
J_h_avg = Γ_h*ω13*mean((n_h+1)*(p3_avg[steady_start_idx:end] .+ p3_2_avg[steady_start_idx:end]) - n_h*(p1_avg[steady_start_idx:end] .+ p1_2_avg[steady_start_idx:end]))
J_m_avg = -sqrt(2)*g*mean(numc_avg[steady_start_idx:end].*pm_avg[steady_start_idx:end])/ω13
J_cold_avg = Γ_c*ω23*mean((n_c+1)*(p3_avg[steady_start_idx:end] .+ p3_2_avg[steady_start_idx:end]) - n_c*(p2_avg[steady_start_idx:end] .+ p2_2_avg[steady_start_idx:end]))
# For cavity: using simplified formula (needs k and n_opt from params if exact)
J_cav_avg = 0.0  # Placeholder - requires more parameters
...
global p1_st = []; # occupation state 1 (atom 1)
global p2_st = []; # occupation state 2 (atom 1)
global p3_st = []; # occupation state 3 (atom 1)

global p1_2_st = []; # occupation state 1 (atom 2)
global p2_2_st = []; # occupation state 2 (atom 2)
global p3_2_st = []; # occupation state 3 (atom 2)
...
update_interval_st = 30.0  # Update every 30 seconds
...
for (step_idx_st, t) in enumerate(tvec)
    ...
    global ρ, exp_numc, xm, pm, j
    # Total jump probability from both atoms
    pj = Γ_c*(n_c+1)*δt*real(expect(p3+p3_2,ρ));
    if pj < rand()
        ...
        #jump happening - determine which atom jumped based on relative probabilities
        p3_1_val = real(expect(p3, ρ))
        p3_2_val = real(expect(p3_2, ρ))
        if rand() < p3_1_val/(p3_1_val + p3_2_val)
            # Jump from atom 1
            ρ = δt*Jc[1]*ρ*dagger(Jc[1])/pj;
        else
            # Jump from atom 2
            ρ = δt*Jc[2]*ρ*dagger(Jc[2])/pj;
        end
    ...
    if t > j*Δt
        ...
        push!(p1_2_st,real(expect(p1_2, ρ)))
        push!(p2_2_st,real(expect(p2_2, ρ)))
        push!(p3_2_st,real(expect(p3_2, ρ)))
...
# Calculate integrated heat for the conditional trajectory
ω13 = params["w_hot"]  # Hot bath transition frequency
ω23 = α * ω13  # Cold bath transition frequency (α=2 defined at top)

# Compute heat currents by integrating over trajectory - sum over both atoms
Q_h_total = 0.0
Q_m_total = 0.0
for (idx, t_val) in enumerate(tplot_st[1:end-1])
    global Q_h_total, Q_m_total
    dt_step = tplot_st[idx+1] - t_val
    Q_h_total += dt_step * Γ_h * ω13 * ((n_h+1)*(p3_st[idx] + p3_2_st[idx]) - n_h*(p1_st[idx] + p1_2_st[idx]))
    Q_m_total += dt_step * (-sqrt(2)*g*numc_st[idx]*pm_st[idx]/ω13)
end

# Heat currents (time-averaged)
total_time_st = tplot_st[end]
J_h_st = Q_h_total / total_time_st
J_m_st = Q_m_total / total_time_st

# Additional heat quantities for compatibility
Q_h = Q_h_total
Q_h_f = Q_h_total  # Can be refined if full Hamiltonian definition is needed

# Cold and cavity currents (add similar integration if needed)
J_cold_st = 0.0  # Placeholder
J_cav_st = 0.0   # Placeholder
...
if ur == 0
    ...
    p2 = Float64.(p2_avg)
    p3 = Float64.(p3_avg)
    p1_2 = Float64.(p1_2_avg)
    p2_2 = Float64.(p2_2_avg)
    p3_2 = Float64.(p3_2_avg)
    na = Float64.(numc_avg)
...
if ur == 1
    ...
    t_p1_2_down = Float64.(p1_2_st)
    t_p2_2_down = Float64.(p2_2_st)
    t_p3_2_down = Float64.(p3_2_st)
    ...
    results = Dict(
        ...
        "t_p2_down" => t_p2_down,
        "t_p3_down" => t_p3_down,
        "t_p1_2_down" => t_p1_2_down,
        "t_p2_2_down" => t_p2_2_down,
        "t_p3_2_down" => t_p3_2_down,
        "t_na_down" => t_na_down,
