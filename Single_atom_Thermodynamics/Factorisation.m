...
stvec=floor(tmax/dt);
stlimit=floor(50*pi/(w_m*dt));%This is only a fraction of the trajectory, where we believe the limit cycle already is achieved.
x_m_vec=zeros(1,stlimit);%This is to check if we have a limit cycle
p_m_vec=zeros(1,stlimit);%This is to check if we have a limit cycle
jump_times=[];
t_p1_old=p1;
t_p2_old=p2;
t_p3_old=p3;
...
J_m=0;%The mechanical heat current, shouldbe (much) smaller than J_h for things to work out.
Q_m=0;
Q_cold=0;
Q_cav=0;
