...
# Compute heat currents (scalar, time-averaged)
J_h_avg = Γ_h*ω13*mean((n_h+1)*p3_avg[steady_start_idx:end] - n_h*p1_avg[steady_start_idx:end])
J_m_avg = -sqrt(2)*g*mean(numc_avg[steady_start_idx:end].*pm_avg[steady_start_idx:end])
J_cold_avg = Γ_c*ω23*mean((n_c+1)*p3_avg[steady_start_idx:end] - n_c*p2_avg[steady_start_idx:end])
...
update_interval_st = 30.0  # Update every 30 second
...
for (step_idx_st, t) in enumerate(tvec)
    ...
    global ρ, exp_numc, xm, pm, j
    pj = Γ_c*(n_c+1)*δt*real(expect(p3,ρ));
    if pj < rand()
        ...
        # # Check if atom is in state |2> with probability 1
        # prob_state2 = real(expect(p2, ρ))
        # if abs(prob_state2 - 1.0) > 1e-6
        #     @warn "After jump at t=$t: Atom is NOT in state |2> with probability 1. P(|2>) = $prob_state2"
        # else
        #     @info "✓ Jump at t=$t: Atom correctly in state |2> with P(|2>) = $prob_state2"
        # end
        
        # # Check if trace of density matrix is 1
        # ρ_trace = real(tr(ρ))
        # if abs(ρ_trace - 1.0) > 1e-6
        #     @warn "After jump at t=$t: Trace of density matrix is NOT 1. Tr(ρ) = $ρ_trace"
        # else
        #     @info "✓ Jump at t=$t: Trace correctly normalized with Tr(ρ) = $ρ_trace, n_c = $n_c, pj = $pj"
        # end
    end
...
# Compute heat currents by integrating over trajectory
Q_h_total = 0.0
Q_m_total = 0.0
for (idx, t_val) in enumerate(tplot_st[1:end-1])
    global Q_h_total, Q_m_total
    dt_step = tplot_st[idx+1] - t_val
    Q_h_total += dt_step * Γ_h * ω13 * ((n_h+1)*p3_st[idx] - n_h*p1_st[idx])
    Q_m_total += dt_step * (-sqrt(2)*g*numc_st[idx]*pm_st[idx])
end
