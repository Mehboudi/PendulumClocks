...
threshholddown=max(.25*sqrt(iM+1));
weight=.0;
...
%%%
% Takethe median as the time of a click, instead of first click (bad
% choice) or average of a sequence (requires knowledge of time in the
% background). Median is only based on the counting (or the current if you
% want), it counts the number of picks, and takes the middle one as the
% time of the tick.
%%%
% itthold=[];
% for itt=1:length(I2)
%     if and(I2(1,itt)>threshholdup,index==0)
%         tvec_dN1_I2=[tvec_dN1_I2,tt(1,itt)];
%         index=1;
%     elseif and(I2(1,itt)<=threshholddown,index==1)
%         index=0;
%         index2=0;
%         tick_itt=itthold(ceil(length(itthold/2)));
%         %tvec_dN1_I2=[tvec_dN1_I2,tt(1,tick_itt)];%time of event = middle tick among the bunch within the same resonance
%         %tvec_dN1_I2=[tvec_dN1_I2,tt(1,itt)];%time of event = below
%         %threshold time
%         %tvec_dN1_I2=[tvec_dN1_I2,tt_dumy];% time of event=average of above
%         %threshold time
%         tt_dumy=0;
%         itthold=[];
%     end
%     if and(I2(1,itt)>threshholdup,index==1)
%         itthold=[itthold,itt];
%         tt_dumy=(index2*tt_dumy+tt(1,itt))/(1+index2);
%         index2=index2+1;
%     end
% end
...
if plot_filter==1
    figure(30)
    subplot(1,6,iM)
    % Define the lower and upper bounds of the plot
    l = 0;
    u = 5.1;
    ...
    %
    plot(x(indices),I2(indices),'LineWidth',2)
    current_ylim = ylim;
    hold on
    xline(tvec_dN1_limit,'LineWidth',.01,'LineStyle','--','Color','r','HandleVisibility','off')
